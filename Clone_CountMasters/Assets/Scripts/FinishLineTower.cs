...
using UnityEngine;
using DG.Tweening;
public class FinishLineTower : MonoBehaviour
{
    ...
    public static FinishLineTower instance;

    private void Start() {
        instance = this;
    }

    public void CreateTower(int stickmanNumber)
    {
        playerAmount = stickmanNumber;
        FillTowerList();
    ...
    IEnumerator BuildTowerCoroutine()
    {
        ...
        foreach (int  towerHumanCount in towerCountList)
        {
            ...
            {
                child.transform.DOLocalMove(child.transform.localPosition + new Vector3(0f , yGap , 0f) , 0.2f).SetEase(Ease.Flash);
            }
            ...
            for(int i = 1 ; i < transform.childCount ; i++)
            {
                ...
                child.transform.localPosition = new Vector3(tempTowerHumanCount * xGap , 0f , 0f);
